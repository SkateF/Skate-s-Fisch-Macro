...
Gui, Add, Checkbox, vAutoNight x16 y70 w121 h20, Auto Night
Gui, Add, Checkbox, vDontRelyOnNoCameraShake x16 y103 w121 h30, Do not rely on no Camera Shake
...
== IF YOU HAVE AUTO NIGHT ON MAKE SURE SUNDIAL TOTEMS ARE IN 8TH SLOT
== IF Do not rely on no Camera Shake is on, it assumes you DO NOT have no camera shake on in game
== IF Do not rely on no Camera Shake is off, it assumes you DO have no camera shake on in game and start
...
Gui, Tab, 6
Gui, Add, GroupBox, x8 y8 w455 h100, Config Management
...
Gui Show, w470 h245, Skate's Macro V1.2.7
Return
...
SaveAllSettings:
    ...
    IniWrite, % (AutoNight ? "true" : "false"), %SelectedFile%, Farm, AutoNight
...
RunReadConfigCode:
    ...
    IniRead, AutoNight, %ConfigFile%, Farm, AutoNight, false
    GuiControl,, AutoNight, % (AutoNight = "true" ? "1" : "0")
