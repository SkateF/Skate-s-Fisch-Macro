...
ConfigFolder := LoadConfig
	...
	IniRead, AutoNight, %ConfigFolder%, Farm, AutoNight, false
	if (FarmCanal = "true" && AutoNight = "false") {
		AutoNight := false
		FarmCanal := true
		AutoEnableCameraMode := true
		Webhook := true
	} 
	if (AutoNight = "true" && FarmCanal = "true" ) {
		FarmCanal := true
		AutoEnableCameraMode := true
		Webhook := true
		AutoNight := true
	...
	IniRead, DontRelyOnNoCameraShake, %ConfigFolder%, Farm, DontRelyOnNoCameraShake, false
...
RestartMacro:
    ...
    If (timeSinceLastRun >= runInterval || lastRunTime == 0) {
    	...
    	if (FarmCanal == true) {
            ...
            If (DontRelyOnNoCameraShake == true)
				{
					Loop
					{
...
if (ErrorLevel == 0)
	...
	goto BarMinigame2
		...
		if (FarmCanal == true && Webhook == true && AutoEnableCameraMode == true && AutoNight == true) {
			sleep, 300
			Click, left, %X%, %Y%
			Send, z
			sleep, 300
			...
			if (AutoNight == true) {
				...
				sleep, 100
				ImageSearch, FoundX, FoundY, LeftX, TopY, RightX, BottomY, *28 %ImagePath%
				if (ErrorLevel = 0) {
					click, Left, %middleX%, %middleY%
					click, Left, %middleX%, %middleY%
					click, Left, %middleX%, %middleY%
		...
		if (FarmCanal == true && Webhook == true && AutoEnableCameraMode == true && AutoNight == false) {
			sleep, 300
			Click, left, %X%, %Y%
			sleep, 300
			Click, left, 500, 500
			...
			Click, left, %X%, %Y%
			sleep, 300
		if (FarmCanal == false && Webhook == true && AutoEnableCameraMode == false) {
			Send, z
			sleep, 600
		}
	}
